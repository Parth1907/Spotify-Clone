...
const loadPlaylist = async (endpoint, elementId) => {
	...
	for (let {name, description, images, id} of items) {
		...
		playlistItem.className =
			"bg-black_secondary rounded p-4 hover:cursor-pointer hover:bg-light_black";
		playlistItem.id = id;
...
const fillContentForDashboard = () => {
	...
	coverContent.innerHTML = `<h1 class="text-6xl" style="width: 500px;">Hello, ${displayName}</h1>`;
...
const playTrack = (
	event,
	{image, artistNames, name, duration_ms, previewURL, id}
) => {
...
const loadPlaylistTracks = ({tracks}) => {
	...
	for (let trackItem of tracks.items.filter((item) => item.track.preview_url)) {
		...
		track.id = id;
		track.className =
			"track p-1 grid grid-cols-[50px_1fr_1fr_50px] gap-4 text-secondary items-center justify-items-start rounded-md hover:bg-light_black";
...
const fillContentForPlaylist = async (playlistId) => {
	...
	<img  class="object-contain h-56 w-56" src="${images[0].url}" alt="" />
	<section>
		<h2 id="playlist-name" class="text-4xl">${name}</h2>
		<p id="playlist-details">${description}</p>
		<p id="playlist-songs">${tracks.items.length} songs</p>
	</section>`;
...
const onContentScroll = (event) => {
	...
	if (history.state.type === SECTIONTYPE.PLAYLIST) {
		...
		if (coverOpacity <= 35) {
			playlistHeader.classList.add("sticky", "bg-black_secondary", "px-8");
			playlistHeader.classList.remove("mx-8");
			playlistHeader.style.top = `${header.offsetHeight}px`;
		} else {
			playlistHeader.classList.remove("sticky", "bg-black_secondary", "px-8");
...
document.addEventListener("DOMContentLoaded", async () => {
	...
	// 	playlist: "37i9dQZF1DX3TRdW1ekdvM",
	// };
	...
	audio.addEventListener("play", () => {
		...
		progressInterval = setInterval(() => {
			...
			songDurationCompleted.textContent = `0:${
				// the following code condition is only for the preview url i.e. for 30 sec playback
